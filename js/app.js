...
function UpdateData() {
  ...
  $.when(
    $.get(
      "/ibi_apps/run.bip?BIP_REQUEST_TYPE=BIP_RUN&BIP_folder=IBFS%253A%252FEDA%252FEDASERVE%252Ftypeahead&BIP_item=" +
        procedure +
        "&windowHandle=436960&IBI_random=4516.2870024981075&SEGDESC=" +
        segdesc,
      function(data) {
    ...
    $.get(
      "/ibi_apps/run.bip?BIP_REQUEST_TYPE=BIP_RUN&BIP_folder=IBFS%253A%252FEDA%252FEDASERVE%252Ftypeahead&BIP_item=procedure2.fex&windowHandle=271353&IBI_random=2165.7337772878413",
      function(data) {
        parse = JSON.parse(data);
        ...
        null,
        {
          ...
          templates: {
            ...
            suggestion: function(data) {
              ...
              if (data.TBNAME) {
                _suggestion =
                  "<div>" + data.value + " in " + data.TBNAME + "</div>";
              } else {
                _suggestion = "<div>" + data.value + "</div>";
              }
              return _suggestion;
            }
          }
  ...
  $("#actionvar").on("tokenfield:createdtoken", function(event) {
    configureBkgColor(event);
    //   clearAllMainTypeaheadTokens();
  });

  $("#byfield").on("tokenfield:createdtoken", function(event) {
    configureBkgColor(event);

    // clearAllMainTypeaheadTokens();
  });

  $("#wherefield").on("tokenfield:createdtoken", function(event) {
    configureBkgColor(event);
...
function getKeywordPosAndDeleteTillNextKeyword(keywordIndex) {
  ...
  if (tokens.length > 0) {
    ...
    if (keywordPosArr.length >= 1) {
      ...
      to = keywordPosArr[keywordIndex + 1]
        ? keywordPosArr[keywordIndex + 1]
        : tokens.length;
      if (from === to - 1) {
    ...
    if (keywrdPosArr[0] === tokens.length - 1) {
      ...
      for (var i = keywrdPosArr[0]; i < tokens.length - 1; i++) {
        if (tokens[keywrdPosArr[0]].value === "COUNT OF") {
          ...
          result_str +=
            " " + tokens[keywrdPosArr[0]].value + " " + enteredVal[i + 1];
        }
...
function button1_onclick(event) {
    ...
    for (var l = 0; l < keywordBuilderArr.length; l++) {
      if (keywordBuilderArr[l].startsWith(" BY")) {
        ...
        if (splittedWhereArr.includes("WHERE")) {
          for (
            var _whereindex = 0;
            _whereindex < splittedWhereArr.length;
            _whereindex++
          ) {
            if (
              splittedWhereArr[_whereindex] !== "" &&
              splittedWhereArr[_whereindex] !== "WHERE"
            ) {
              tempWhere.push({
                TBNAME: "employee",
        ...
        for (
          var tempWhereIndex = 0;
          tempWhereIndex < tempWhere.length;
          tempWhereIndex++
        ) {
        ...
        if (
          enteredStringArr.indexOf("IS EQUAL") ===
          enteredStringArr.indexOf("WHERE") + 2
        ) {
          _whereStr +=
            " EQ " +
            "'" +
            enteredStringArr[enteredStringArr.indexOf("WHERE") + 3] +
            "'";
        ...
        if (
          enteredStringArr.indexOf("IS GREATER THAN") ===
          enteredStringArr.indexOf("WHERE") + 2
        ) {
          _whereStr +=
            " GT " +
        ...
        if (
          enteredStringArr.indexOf("IS LESS THAN") ===
          enteredStringArr.indexOf("WHERE") + 2
        ) {
          _whereStr +=
            " LT " +
        ...
        if (
          enteredStringArr.indexOf("IS LESS THAN OR EQUAL TO") ===
          enteredStringArr.indexOf("WHERE") + 2
        ) {
          _whereStr +=
            " LE " +
        ...
        if (
          enteredStringArr.indexOf("IS GREATER THAN OR EQUAL TO") ===
          enteredStringArr.indexOf("WHERE") + 2
        ) {
          _whereStr +=
            " GE " +
        ...
        if (
          enteredStringArr.indexOf("IS NOT EQUAL TO") ===
          enteredStringArr.indexOf("WHERE") + 2
        ) {
          _whereStr +=
            " NE " +
        ...
        if (splittedCNTArr.includes("CNT")) {
          for (
            var _CNTindex = 0;
            _CNTindex < splittedCNTArr.length;
            _CNTindex++
          ) {
            if (
              splittedCNTArr[_CNTindex] !== "" &&
              splittedCNTArr[_CNTindex] !== "CNT"
            ) {
              tempCNT.push({
                TBNAME: "employee",
        ...
        if (enteredStringArr.indexOf("WHERE") > -1) {
          if (
            enteredStringArr.indexOf(keywordBuilderArr[l]) ===
              enteredStringArr.indexOf("WHERE") + 3 ||
  ...
  var _url =
    "/ibi_apps/run.bip?BIP_REQUEST_TYPE=BIP_RUN&BIP_folder=IBFS%253A%252FWFC%252FRepository%252Ftypeahead&BIP_item=procedure_submit_2.fex&windowHandle=639047&IBI_random=1158.9034483962823";
  ...
  var dynamicurl =
    "&FEXTYPE=GRAPH&DATABASE=" +
    dbname +
    "&FORMATTYPE=JSCHART&ACTION=" +
    _action +
    "&ACTIONVARIABLE=" +
    _actionVar +
    "&BYSTRING=" +
    ...
    if (where_tokens.length > 0) {
      for (var w = 0; w < where_tokens.length; w++) {
        ...
        if (where_tokens[w].value === "IS EQUAL") {
          ...
          where_tokens[w].value &&
          (where_tokens[w].TBNAME === "employee" ||
            where_tokens[w].TBNAME === "") &&
          where_tokens[w].TBNAME !== undefined
        ) {
...
var _url =
  "/ibi_apps/run.bip?BIP_REQUEST_TYPE=BIP_RUN&BIP_folder=IBFS%253A%252FWFC%252FRepository%252Ftypeahead&BIP_item=procedure_submit_2.fex&windowHandle=410540&IBI_random=1639.8353503362162";
var _ibiapp = "typeahead/";
    ...
    success: function(_data) {
      ...
      if (isError > -1) {
        $("#iframe2")
          .contents()
          .find("body")
          .append("Sorry! Results not found");
        IbComposer_triggerExecution("Hideiframe", 1);
